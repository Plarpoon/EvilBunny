...
export class VoiceStateUpdateListener extends Listener {
    ...
    public run(oldState: VoiceState, newState: VoiceState) {
        ...
        if (!oldState.channelId && newState.channelId) {
            ...
            channel.send(`${newState.member?.user.tag} left ${oldState.channel?.name}`);
        } else if (oldState.channelId && newState.channelId) {
            channel.send(`${newState.member?.user.tag} moved from ${oldState.channel?.name} to ${newState.channel?.name}`);
        }
    }
}
